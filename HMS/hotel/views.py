...
def payment(request):
    ...
    booking = Booking.objects.filter(pk=request.session.get("booking_id")).first()
    ...
    def send(request, receiver, code):
        ...
        # OSError also covers connection refusal and socket errors.
        try:
            send_mail(
            ...
            if settings.DEBUG:
                print("Payment verification code:", code)
                messages.warning(request, "Failed to send email verification. Code printed to terminal.")
            else:
                messages.warning(request, "Failed to send email verification. Please retry payment to resend the code.")
        else:
            messages.success(
                request, 'Verification email Was Successfully Sent')

    send(request, booking.guest, code)
    request.session["payment_code"] = code

    return render(request, path + "/payment.html", {"role": role})
...
def verify(request):
    ...
    path = role + "/"
    booking = Booking.objects.filter(pk=request.session.get("booking_id")).first()
    code = request.session.get("payment_code")
    if booking is None:
        request.session.pop("booking_id", None)
        request.session.pop("payment_code", None)
        messages.warning(request, "Please select a room and check-in/check-out dates.")
        return redirect("rooms")
    if not code:
        messages.warning(request, "Please initiate payment before verification.")
        return redirect("payment")

    if request.method == "POST" and "verify" in request.POST:
        if request.POST.get("realCode") == code:
